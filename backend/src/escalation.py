...
from src.models import RiskLevel, EscalationPath, GraphNode, GraphLink, EffectivePermission, PolicyEffect, RelationshipType
...
def detect_escalation_paths(graph, effective_permissions_map: Dict[str, List[EffectivePermission]]) -> List[EscalationPath]:
    ...
    for start_id in nodes_map:
        ...
        while queue:
            ...
            for rule in RULES:
                rule_satisfied = True
                for req_group in rule.required:
                    group_satisfied = any(has_permission(perms, act) for act in req_group)
                    if not group_satisfied:
                        rule_satisfied = False
                        break
                
                if rule_satisfied:
                    current_node = nodes_map[start_id]
                    rule_score = get_risk_score(rule.risk)
                    if rule_score > current_node.risk_score:
                        current_node.risk_score = rule_score
                        current_node.risk_level = rule.risk
                    
                    esc_id = str(uuid.uuid4())
                    current_node.escalation_paths.append(esc_id)
                    ...
                    paths.append(EscalationPath(
                        ...
                        risk=rule.risk,
                        path=path_so_far,
                        required_permissions=[act for group in rule.required for act in group],
